...
Let us notice that this time there is no need to use pseudo inverse, because $\boldsymbol{X}^T\boldsymbol{X}+\lambda\boldsymbol{I}$ is invertible. The proof of this fact can be found in \Cref{app:reg}. The learning procedure of regularized LR classifier is shown in \Cref{alg:LR2}
...
\subsection{Random PCA method}

Let us consider randomized version of PCA. The procedure is presented in \Cref{alg:rpca1} (\cite{Mineiro}). 
\begin{algorithm}
...
Although there is no point in using pure CCA to build a classifier, we can use methods which are based on CCA concept. Such the algorithm will be discussed in the next section.
...
\end{equation}
A matrix $\boldsymbol{H}=\boldsymbol{X}\boldsymbol{X}^P$ is sometimes called a hat matrix. The presented approach is called \textit{Orthogonally constraint CCA (OCCA)}. In order to solve \Cref{eq:cplst4}, we can consider eigenvectors that correspond to the largest eigenvalues of $\boldsymbol{Y}^T(\boldsymbol{H}-\boldsymbol{I})\boldsymbol{Y}$ (they can be computed by SVD decomposition) (\cite{ChenLin}).
